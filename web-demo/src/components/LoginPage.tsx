...
interface LoginPageProps {
  ...
  users: { username: string; email: string; password: string; type: UserRole }[];
}

export default function LoginPage({ onLogin, users }: LoginPageProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const found = users.find(
      (rec) => (rec.email === input || rec.username === input) && rec.password === password,
    );
    ...
    try {
      const nameMap: Record<UserRole, string> = {
        ...
        noibo: 'Nhân sự Nội bộ',
        phongtuyenthong: 'Phòng Truyền Thông',
      };
      ...
      const payload: AuthUser = {
        id: idMap[found.type],
        name: nameMap[found.type],
        email: found.email,
        role: found.type,
      };
      // Lưu vào localStorage
      localStorage.setItem('auth', JSON.stringify(payload));
      // Có thể lưu thêm cờ rememberMe nếu cần
      localStorage.setItem('rememberMe', JSON.stringify(rememberMe));
      onLogin(payload);
    } catch (err) {
      console.log('err', err);
      setError('Không thể lưu dữ liệu đăng nhập');
    }
  };
        ...
        <div className="w-full max-w-md">
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-gray-200 p-10">
            <div className="mb-8 text-center">
              <h2 className="text-gray-600 mb-3">Đăng nhập hệ thống</h2>
              <p className="text-gray-600" style={{ fontSize: 'var(--text-base)' }}>Nhập thông tin để truy cập CPMS</p>
            </div>
          ...
          <p className="text-center text-gray-500 mt-6 text-sm">
            © 2025 Vietnam Airlines. All rights reserved.
          </p>
        </div>
      </div>
    </div>
